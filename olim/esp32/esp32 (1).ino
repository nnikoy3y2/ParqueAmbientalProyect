...
// ─── RECONEXIÓN WIFI (no bloqueante, con backoff) ─────────────────
unsigned long tiempoUltimoIntentoWifi = 0;
unsigned long intervaloReconexionWifi = 2000;      // arranca en 2 seg
const unsigned long intervaloReconexionMax = 30000; // techo: 30 seg
bool wifiEstabaCaido = false;
...
// Intenta reconectar sin bloquear el loop. Usa backoff simple: si falla,
// duplica la espera hasta un techo (intervaloReconexionMax) para no
// spamear WiFi.begin() sin sentido.
void manejarReconexionWifi() {
  if (WiFi.status() == WL_CONNECTED) {
    if (wifiEstabaCaido) {
      Serial.println("✅ WiFi reconectado. IP: " + WiFi.localIP().toString());
      wifiEstabaCaido = false;
      intervaloReconexionWifi = 2000; // reset del backoff al reconectar
    }
    return;
  }

  wifiEstabaCaido = true;

  if (millis() - tiempoUltimoIntentoWifi >= intervaloReconexionWifi) {
    tiempoUltimoIntentoWifi = millis();
    Serial.println("⚠️ WiFi desconectado. Reintentando conexión...");

    WiFi.disconnect();
    WiFi.begin(ssid, password);

    // Backoff exponencial simple, con techo, para el próximo intento
    intervaloReconexionWifi = min(intervaloReconexionWifi * 2, intervaloReconexionMax);
  }
}

void loop() {
  // 0. MANTENER LA CONEXIÓN WIFI VIVA (no bloqueante)
  ...
  if (millis() - tiempoUltimoReporte >= intervaloReporte) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      Serial.println("⏸️ Sin conexión WiFi: dato no enviado (se reintentará solo).");
    }
  }
